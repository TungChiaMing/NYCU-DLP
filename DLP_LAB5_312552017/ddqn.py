...
class DQN:
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        '''
        ε is big initally, because we want to do exploration(find new possible) at the begining,
        while it will be small later, because we want to do exploitation(just use previous choice).
        '''
        ...
        if random_number < epsilon:
            ...
            with torch.no_grad():
                q_value = self._behavior_net(torch.from_numpy(state).to(self.device))
                action = torch.argmax(q_value).item() # q_value.argmax().item()
            return action 
        ## TODO ##
...
def train(args, env, agent, writer):
    ...
    for episode in range(args.episode):
        ...
        if episode % 100 == 0 and episode != 0:
            ...
            agent.save("ddqn"  + str(episode) + ".pth")
...
def main():
    ...
    parser.add_argument('-m', '--model', default='ddqn700.pth')
