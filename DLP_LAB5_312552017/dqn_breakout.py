...
class ReplayMemory(object):
    ...
    def push(self, *transition):
        ...
        # deque.append() takes exactly one argument (5 given)
...
class DQN:
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        if random_number < epsilon:
            ...
            with torch.no_grad():
                ...
                action = torch.argmax(q_value, 1).item() # q_value.argmax().item()
            return action 
    ...
    def append(self, state, action, reward, next_state, done):
        ...
        #self._memory.push(...)
        self._memory.push(state, [action], [reward], next_state, [int(done)]) # 'int' object is not iterable
    ...
    def _update_behavior_network(self, gamma):
        ...
        ## Hint. Stack a sequence of four frames
        state = state.permute(0, 3, 1, 2)
        next_state = next_state.permute(0, 3, 1, 2)


        q_value = self._behavior_net(state)
        q_value_action = q_value.gather(1, action.long())
...
def train(args, agent, writer):
    ...
    env = wrap_deepmind(env_raw, episode_life=True, clip_rewards=True, frame_stack=True) # Hint: Trick 1
    ...
    for episode in range(args.episode):
        ...
        '''
        Test actor every 100 game.
        '''
        if episode % 5000 == 0 and episode != 0:
            test(args, agent, writer)
            agent.save("dqn_breakout" + str(episode) + ".pth")
        ...
        for t in itertools.count(start=1):
            ...
            #     test(args, agent, writer)
            #     agent.save(args.model + "dqn_breakout_ex" + str(total_steps) + ".pth")
...
def test(args, agent, writer):
    print('Start Testing')
    env_raw = make_atari('BreakoutNoFrameskip-v4')
    env = wrap_deepmind(env_raw, frame_stack=True) # Hint: Trick 2
    action_space = env.action_space
    e_rewards = []
...
def main():
    ...
    parser.add_argument('--episode', default=80000, type=int)
    ...
    parser.add_argument('-tmp', '--test_model_path', default='dqn_breakout.pth')
