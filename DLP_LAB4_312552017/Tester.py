...
def calculate_PSNR(save_root, gen_image, idx):
    ...
    Result.plot_psnr(PSNR_LIST, args)
    return sum(PSNR_LIST)/(len(PSNR_LIST)-1)
        

def get_key(fp):
    filename = fp.split('/')[-1]  # if windows,  filename = fp.split('\\')[-1]
    ...
    def load_checkpoint(self):
        if self.args.ckpt_path != None:
            ...
            if args.plot == True:
                ...
                current_epoch = checkpoint['last_epoch']
                ave_psnr_list = checkpoint['ave_psnr']
                loss = checkpoint['train_loss']
                mse_loss = checkpoint['train_mse_loss']
                kld_loss = checkpoint['train_kld_loss']
                kl_weight = checkpoint['train_kl_weight']
                tfr = checkpoint['train_kl_tfr']

                #print(f"best_psnr: {best_psnr}")
                #print(f"current_epoch: {len(current_epoch)}")
                #print(f"ave_psnr_list: {len(ave_psnr_list)}")

                #print(f"training_loss: {len(loss)}")
                #print(f"training_mse_loss: {len(mse_loss)}")
                #print(f"training_kld_loss: {len(kld_loss)}")
                #print(f"training_kl_weight: {(kl_weight)}")
                #print(f"training_tfr: {len(tfr)}")

                Result.plot_loss(current_epoch+1, loss, mse_loss, kld_loss, tfr, arg)
