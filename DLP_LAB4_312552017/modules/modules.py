...
class Gaussian_Predictor(nn.Sequential):
    ...
    def reparameterize(self, mu, logvar):
        '''
        Re-parameterize the encoder (mu and logvar of a distribution)
        so backpropagation will be no problem (original not differentiable)
        express the latent variable z as a deterministic function of the parameters
        and a random variable ϵ, where z = mu + logvar * ϵ, ϵ is sampled from normal distribution
        '''
        # .add_(), .mul_(), .exp_() are for pytorch tensor operations
        
        std_deviation  = logvar.mul(0.5).exp_()  # varience = ( exp(logvar) ) ** 0.5 

        #　generates with the same size as std where each element is drawn from a standard normal distribution.
        epsilon = torch.randn_like(std_deviation)

        trick = epsilon.mul(std_deviation).add_(mu)
        return trick
        # TODO
